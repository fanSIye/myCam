...
//added by zzd, 20171015
  private byte[][] joinParameters(byte[] first, byte[] rest, int length) {
	    byte[][] result = new byte[2][];
	    result[0] = first;
	    result[1] = new byte[length];
	    System.arraycopy(rest, 0, result[1], 0, length);
	    return result;
  }
//==========================  
...
//added by zzd, 20171015
  public void lpush(final byte[] key, final byte[] string, int stringLength) {
	    sendCommand(LPUSH, joinParameters(key, string, stringLength));
  }
  //=========================
  ...
  public void subscribe(final byte[]... channels) {
    sendCommand(SUBSCRIBE_, channels);
  }
  ...
  public void unsubscribe() {
    sendCommand(UNSUBSCRIBE_);
  }

  public void unsubscribe(final byte[]... channels) {
    sendCommand(UNSUBSCRIBE_, channels);
  }

  public void psubscribe(final byte[]... patterns) {
    sendCommand(PSUBSCRIBE_, patterns);
  }

  public void punsubscribe() {
    sendCommand(PUNSUBSCRIBE_);
  }

  public void punsubscribe(final byte[]... patterns) {
    sendCommand(PUNSUBSCRIBE_, patterns);
  }
